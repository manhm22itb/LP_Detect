...
if (len(LpImg)):
    ...
    text = fine_tune(plate_info)

    # Thiết lập vị trí chữ
    x, y = 50, 50

    # Tính kích thước của khung chữ nhật quanh chữ
    (text_width, text_height), baseline = cv2.getTextSize(text, cv2.FONT_HERSHEY_PLAIN, 3.0, thickness=2)

    # Vẽ khung chữ nhật màu trắng (background)
    cv2.rectangle(Ivehicle, (x - 10, y - text_height - 10), (x + text_width + 10, y + baseline + 10), (255, 255, 255), thickness=-1)

    # Viết chữ màu xanh lá cây (BGR: 0, 255, 0)
    cv2.putText(Ivehicle, text, (x, y), cv2.FONT_HERSHEY_PLAIN, 3.0, (0, 255, 0), thickness=2, lineType=cv2.LINE_AA)
